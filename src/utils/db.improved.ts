...
if (useConnectionString) {
  ...
  pool = new Pool({
    ...
    max: 5, // Reduced max connections to prevent connection exhaustion
    allowExitOnIdle: true // Allow the app to exit if all connections are idle
  });
  ...
  pool = new Pool({
    ...
    ssl: { rejectUnauthorized: false },
    connectionTimeoutMillis: 30000, // Increased timeout for Azure
    idleTimeoutMillis: 30000,
    max: process.env.NODE_ENV === 'production' ? 5 : 10, // Reduced max connections for production
...
export async function testConnection() {
  try {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error('Connection timeout: Failed to acquire client from pool')), 10000);
    });
...
export async function getClientWithTimeout(timeoutMs = 5000) {
  const connectPromise = pool.connect();
  const timeoutPromise = new Promise<never>((_, reject) => {
    setTimeout(() => reject(new Error('Connection timeout: Failed to acquire client from pool')), timeoutMs);
  });
  ...
  try {
    ...
    console.error('[DB] Error acquiring client with timeout:', err);
    throw err;
  }
}
