...
async function seed() {
  ...
  await client.query(
    `INSERT INTO ${schema}.fuel_prices (tenant_id, station_id, fuel_type, price, effective_from)
     ...
     ON CONFLICT (station_id, fuel_type, effective_from) DO NOTHING`,
    [tenantId, stationId]
  );

  // sample user activity logs
  await client.query(
    `INSERT INTO ${schema}.user_activity_logs (user_id, ip_address, user_agent, event)
     SELECT id, '127.0.0.1', 'seed-script', 'login'
     FROM ${schema}.users LIMIT 1`
  );

  console.log(`Seeded tenant '${schema}' with sample data including fuel prices and activity logs.`);
